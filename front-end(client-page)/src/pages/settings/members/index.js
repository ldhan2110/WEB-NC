...
import {
  Add as AddIcon, SettingsInputAntennaTwoTone,
...
const MemberListPage = (props) => {
  ...
  useEffect(()=>{
    ...
    axios.get("http://localhost:8000/admin/users").then(function (res){
      const listMembers=[];
      for (var data of res.data) {
        let member={};
        member.id=data._id;
        member.name=data.usr_fullname;
        member.email=data.usr_email;
        member.role=data.usr_role.charAt(0).toUpperCase() + data.usr_role.slice(1);
        member.status=data.active ? 1:0;
